...
class elementInvalidNodesUpdate(BaseApi):
    ...
    def add_element_invalid_nodes2(self, token, product_type, ppp_serial, pc_serial, num):
        """
        过程特性下添加要素失效--从要素失效库选择多个要素失效
        :param num: 添加要素失效的个数-3
        """
        self.token = token
        res = getElementInvalid().get_element_invalid(token, product_type)
        ...
        for i in range(num):
            data["json"].append(
                {"enInvalidModeName": res[i]["enInvalidMode"], "invalidmodeName": res[i]["invalidmode"],
                 "invalidmodeSerial": res[i]["serialNum"],
                 "occurrence": res[i]["occurrence"], "pppSerial": ppp_serial, "projectFeatureSerial": pc_serial})
        res = self.send(data)
        if res.status_code != 200:
            return False
        if res.json()["meta"] and res.json()["meta"]["success"] != True:
    ...
    def del_element_invalid_nodes(self, token, ppp_serial,serial_num):
        ...
        data = {
            "method": "post",
            "url": f"/gateway/fmea-pfmea/pfmeaProjectInvalid/delete",
            "json": {
                "invalidModeSerial": "",
